  ...
  useEffect(()=>{
    const fetchItemsInfo= async() =>{
      ...
      try 
      {
        ...
        {
          const fruits = response.data.data.filter((item) => item.type === 'fruit');
          setItems(fruits);
        }
      } 
      catch (error) 
    ...
    <div>
      ...
      {(selected===null) &&
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 sm:px-5 px-2 py-5 overflow-hidden bg-gradient-to-b from-[#FFFBE6]
          to-[#347928]">
            {items.map((item) => (
